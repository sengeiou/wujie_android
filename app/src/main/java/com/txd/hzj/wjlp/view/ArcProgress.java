...
public class ArcProgress extends View {
    ...
    private String progressSuffixText2 = "";
    ...
    public ArcProgress(Context context, AttributeSet attrs, int defStyleAttr) {
        ...
        mContext = context;
        default_text_size = ToolKit.sp2px(context, 18);
        min_size = (int) ToolKit.dip2px(context, 100);
        default_text_size = ToolKit.sp2px(context, 40);
        default_suffix_text_size = ToolKit.sp2px(context, 15);
        default_suffix_padding = ToolKit.dip2px(context, 4);
        default_suffix_text = "%";
        default_bottom_text_size = ToolKit.sp2px(context, 10);
        default_stroke_width = ToolKit.dip2px(context, 4);
        TypedArray attributes = context.getTheme().obtainStyledAttributes(attrs, R.styleable.ArcProgress, defStyleAttr,
                0);
    ...
    protected void initByAttributes(TypedArray attributes) {
        ...
        // 进度条上面字体颜色
        progressTextTopColor = attributes.getColor(R.styleable.ArcProgress_arc_progress_text_top_color,
                default_text_color);
        // 进度条上面字体大小
        progressTextTopSize = attributes.getDimension(R.styleable.ArcProgress_arc_progress_text_top_size,
                default_text_size);
        // 进度条上面
        ...
        // 进度条下面字体颜色
        progressTextBottomColor = attributes.getColor(R.styleable.ArcProgress_arc_progress_text_bottom_color,
                default_text_color);
        // 进度条下面字体大小
        progressTextBottomSize = attributes.getDimension(R.styleable.ArcProgress_arc_progress_text_bottom_size,
                default_text_size);
        ...
        progressSuffixTextColor = attributes.getColor(R.styleable.ArcProgress_arc_progress_suffix_text_color,
                default_text_color);
        // 旁边字体的宽度
        progressSuffixTextSize = attributes.getDimension(R.styleable.ArcProgress_arc_progress_suffix_text_size,
                default_text_size);
    ...
    protected void onDraw(Canvas canvas) {
        ...
        if (!TextUtils.isEmpty(text)) {
            ...
            Typeface fontFace = Typeface.createFromAsset(mContext.getAssets(), "fonts/t1.otf");
            ...
            if (!TextUtils.isEmpty(getProgressTextTop())) {
                ...
                canvas.drawText(getProgressTextTop(), (getWidth() - textPaint.measureText(getProgressTextTop())) / 2.0f,
                        textBaseline + textHeight - ToolKit.dip2px(mContext, 20), textPaint);
            }
            if (!TextUtils.isEmpty(getProgressTextBottom())) {
                ...
                canvas.drawText(getProgressTextBottom(),
                        ...
                        textBaseline - textHeight / 2 + ToolKit.dip2px(mContext, 5), textPaint);
            }
        }
        ...
        if (!TextUtils.isEmpty(getBottomText())) {
            ...
            float textHeight = textPaint.descent() + textPaint.ascent();
            float textBaseline = (getHeight() - textHeight) / 2.0f;
            canvas.drawText(getBottomText(), (getWidth() - textPaint.measureText(getBottomText())) / 2.0f,
