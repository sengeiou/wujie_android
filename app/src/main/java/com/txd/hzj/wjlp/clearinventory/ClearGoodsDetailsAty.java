...
import com.txd.hzj.wjlp.mellonLine.gridClassify.PosterAty;
...
import static com.ants.theantsgo.util.TimeUtils.DATE_FORMAT_DATE;

/**
 * 创建者：zhangyunfei
 * 创建时间：2019/3/28 11:43
 * 功能描述：
 */
public class ClearGoodsDetailsAty extends BaseAty implements ObservableScrollView.ScrollViewListener, ObservableScrollView.onBottomListener, ProUrbAreaUtil.CallBack, CommodityDetailsInter.CommodityView {
...
//        goods_for_my_evaluste_layout.setVisibility(View.GONE);
    ...
    public void onClick(View v) {
        ...
        switch (v.getId()) {
            ...
            case R.id.title_details_layout:// 详情
                ...
                PosterAty.getInstance(ClearGoodsDetailsAty.this,"1",goods_id,imageUrls,mGoods_name,mIntegral,mTicket_buy_discount,mShop_price,mMarket_price,"",share_content,mSell_num,"");
                break;
            ...
            case R.id.layout_djq:
                commodityDetailsPranster.showDjqPop(v, dj_ticket, ClearGoodsDetailsAty.this, vouchers_desc);
                break;
            case R.id.tv_showClassify:
                toClassify(v, goodsInfo.get("top_cate_id"));
                break;
            case R.id.tv_quxiao://促销弹框
                commodityDetailsPranster.showCXPop(v, ClearGoodsDetailsAty.this, promotionBeen);
                //                showCXPop(v);
                break;
            case R.id.tv_lingquan:
                commodityDetailsPranster.showLQPop(v, "领券", ClearGoodsDetailsAty.this, theTrickAdapter);
                //                showLQPop(v, "领券");
                break;
            case R.id.tv_wjsd:
    ...
    ImageListener imageListener = new ImageListener() {
        ...
        public void setImageForPosition(final int position, ImageView imageView) {
            Glide.with(ClearGoodsDetailsAty.this).load(image.get(position).get("path"))
                    .centerCrop()
    ...
    protected int getLayoutResId() {
        return R.layout.activity_clear_goods_details;
    }
    ...
    void goodsInfo(String goods_id, int p, String order_id,String product_id,BaseView baseView) {
        ...
        apiTool2.postApi(Config.SHARE_URL + "index.php/Api/Goods/goodsInfo", params, baseView);
    }
...
//        goodsInfo(ticket_buy_id, page,"","",this);
    ...
    private View createHeaderView() {
        View headerView = LayoutInflater.from(mContext).inflate(R.layout.layout_head, null);
    ...
    public void onComplete(String requestUrl, String jsonStr) {
        ...
        if (requestUrl.contains("goodsInfo")) {
            ...
            ArrayList<Map<String, String>> firstValist = JSONUtils.parseKeyAndValueToMapList(first_val);
            if (firstValist != null && firstValist.size()>0){
                Map<String, String> f = firstValist.get(0);
                priceTv1.setText("¥"+f.get("wholesale_price"));
                priceTv2.setText("¥"+f.get("supply_price"));
                priceTv3.setText("¥"+f.get("profit"));
            }
            ...
            if (goodsInfo.containsKey("buy_status") && goodsInfo.get("buy_status").equals("0")) {
                ...
                dialog.setPositiveButton("确定", new DialogInterface.OnClickListener() {
                    ...
                    public void onClick(DialogInterface dialog, int which) {
                        ClearGoodsDetailsAty.this.finish();
                    }
                });
                dialog.setCancelable(false);
                dialog.create().show();
            ...
            if (ToolKit.isList(data, "goods_server")) {
                ...
                rv_service.setAdapter(new ClearGoodsDetailsAty.service_adp(ser_list, 3));
            } else {
            ...
            if (ToolKit.isList(data, "comment")) {
                try {
                    Map<String, String> comment = JSONUtils.parseKeyAndValueToMap(data.get("comment"));
                    if (comment.containsKey("total")){
                        all_comment_num_tv.setText("商品评价(" + comment.get("total") + ")");
                        if (Integer.parseInt(comment.get("total")) > 0) { // 如果评论数量大于0（有评论）则显示该模块内容
                            layout_comment.setVisibility(View.VISIBLE);
                            Map<String, String> body = JSONUtils.parseKeyAndValueToMap(comment.get("body"));
                            if (body != null) {
                                Glide.with(this).load(body.get("user_head_pic"))
                                ...
                                comm_user_name_tv.setText(body.get("nickname"));
                                comm_content_tv.setText(body.get("content"));
                                tv_date.setText(body.get("create_time"));
                                List<PicturesBean> pictures = JSONUtils.parseKeyAndValueToMapList(PicturesBean.class,body.get("pictures"));
            ...
            if (ToolKit.isList(data, "goods_common_attr")) {
                ...
                GoodsCommentAttrAdapter gcaAdapter = new GoodsCommentAttrAdapter(ClearGoodsDetailsAty.this, gca);
            ...
            if (!data.get("cheap_group").equals("[]")) {
                ...
                if (cheap_group != null) {
                    ...
                    rv_cheap_group.setAdapter(new ClearGoodsDetailsAty.cg_adp(maps));
                } else {
        ...
        if (requestUrl.contains("addCollect")) {// 添加收藏
            showRightTip("收藏成功");
            is_collect = "1";
            commodityDetailsPranster.isCollect(is_collect, "已收藏", goods_title_collect_tv, ClearGoodsDetailsAty.this);
            return;
        }
        if (requestUrl.contains("delOneCollect")) {
            showRightTip("取消成功");
            is_collect = "0";
            commodityDetailsPranster.isCollect(is_collect, "收藏", goods_title_collect_tv, ClearGoodsDetailsAty.this);
        }
    ...
    public void freightGetEd(Map<String, String> map) {
        L.e("ClearGoodsDetailsAty.map=" + map.toString());
    ...
    class DialogAdapter extends BaseAdapter {
        ...
        public View getView(int position, View convertView, ViewGroup parent) {
            ClearGoodsDetailsAty.DialogAdapter.MyHolder holder;
            if (convertView == null) {
                holder = new ClearGoodsDetailsAty.DialogAdapter.MyHolder();
                convertView = LayoutInflater.from(ClearGoodsDetailsAty.this).inflate(R.layout.item_popup_sel_chat, null);
                convertView.setTag(holder);
                com.lidroid.xutils.ViewUtils.inject(holder, convertView);
            } else {
                holder = (ClearGoodsDetailsAty.DialogAdapter.MyHolder) convertView.getTag();
            }

            EasemobBean.DataBean.EasemobAccountBean easemobAccountBean = list.get(position);

            Glide.with(ClearGoodsDetailsAty.this).load(list.get(position).getHead_pic())
                    .placeholder(R.drawable.ic_default)
                    .error(R.drawable.ic_default)
                    .diskCacheStrategy(DiskCacheStrategy.SOURCE)
...
//    private TextView tv_integral;//积分
    ...
    private void forGoodsInfo(Map<String, String> goodsInfo) {
        ...
        L.e("==========商品详情获取的定位信息===========" + tx);
        // 定位好之后获取运费信息
        Freight.freight(goods_id, tx, String.valueOf(goods_number), product_id, ClearGoodsDetailsAty.this);
        showProgressDialog();
        timeTv.setText("活动时间："+ TimeUtils.getTime(Long.parseLong(goodsInfo.get("examine_time"))*1000,DATE_FORMAT_DATE)+"-"+TimeUtils.getTime(Long.parseLong(goodsInfo.get("deadline"))*1000,DATE_FORMAT_DATE));
        // 商品价格
...
//        ChangeTextViewStyle.getInstance().forTextColor(this, goods_profit_num_tv, "积分" + mIntegral, 2, Color.parseColor("#FF0000"));
        ...
        goods_details_name_tv.setOnLongClickListener(new View.OnLongClickListener() {
            ...
            public boolean onLongClick(View view) {
                ...
                new MikyouCommonDialog(ClearGoodsDetailsAty.this, "长按将复制文字：" + goods_details_name_tv.getText().toString(), "操作提示", "复制", "取消", true)
...
//        }
        ...
        if (ToolKit.isList(goodsInfo, "goods_active")) {
            ...
            lv_qkk.setAdapter(new ClearGoodsDetailsAty.qkk_adapter(qkk_list));
        ...
        commodityDetailsPranster.setBitmap(ClearGoodsDetailsAty.this, goodsInfo.get("country_logo"), im_country_logo);
...
//        merchant_name = mInfo.get("merchant_name");
    ...
    private void getHeight() {
        ...
        vto.addOnGlobalLayoutListener(new ViewTreeObserver.OnGlobalLayoutListener() {
            ...
            public void onGlobalLayout() {
                ...
                limit_goods_details_sc.setScrollViewListener(ClearGoodsDetailsAty.this);

            }
        });
        limit_goods_details_sc.setOnBottomListener(ClearGoodsDetailsAty.this);
    }
        ...
        commonPopupWindow = new CommonPopupWindow.Builder(this)
                ...
                .setViewOnclickListener(new CommonPopupWindow.ViewInterface() {
                    ...
                    public void getChildView(View view, int layoutResId, int position) {
                        ...
                        recyclerview.setLayoutManager(new LinearLayoutManager(ClearGoodsDetailsAty.this, 1, false));
                        recyclerview.setAdapter(new ClearGoodsDetailsAty.service_adp(list, type));
    ...
    class qkk_adapter extends BaseAdapter {
        ...
        public View getView(int i, View view, ViewGroup viewGroup) {
            view = View.inflate(ClearGoodsDetailsAty.this, R.layout.item_lh_qkk, null);
    ...
    class service_adp extends RecyclerView.Adapter<ClearGoodsDetailsAty.service_adp.ViewHolder> {
        ...
        @Override
        public ClearGoodsDetailsAty.service_adp.ViewHolder onCreateViewHolder(ViewGroup parent, int viewType) {
            return new ClearGoodsDetailsAty.service_adp.ViewHolder(LayoutInflater.from(parent.getContext()).inflate(R.layout.item_lh_service, null));
        }

        @Override
        public void onBindViewHolder(ClearGoodsDetailsAty.service_adp.ViewHolder holder, int position) {
            String name = "";
            String desc = "";
            if (this.tpye == 0) {
            ...
            Glide.with(ClearGoodsDetailsAty.this).load(list.get(position).get("icon")).into(holder.im_logo);
    ...
    class cg_adp extends RecyclerView.Adapter<ClearGoodsDetailsAty.cg_adp.ViewHolder> {
        ArrayList<Map<String, String>> list;
        ...
        @Override
        public ClearGoodsDetailsAty.cg_adp.ViewHolder onCreateViewHolder(ViewGroup parent, int viewType) {
            return new ClearGoodsDetailsAty.cg_adp.ViewHolder(LayoutInflater.from(parent.getContext()).inflate(R.layout.item_rv_dpg, null));
        }

        @Override
        public void onBindViewHolder(ClearGoodsDetailsAty.cg_adp.ViewHolder holder, int position) {
            Glide.with(ClearGoodsDetailsAty.this).load(list.get(position).get("goods_img")).into(holder.imageview);
...
//                        "积分" + data.getStringExtra("red_return_integral"), 2, Color.parseColor("#FF0000"));
    ...
    public void onRequestPermissionsResult(int requestCode, @NonNull String[] permissions, @NonNull int[] grantResults) {
        ...
        AndPermission.onRequestPermissionsResult(requestCode, permissions, grantResults, commodityDetailsPranster.requestPhoneListener(merchant_phone, ClearGoodsDetailsAty.this));
    }
