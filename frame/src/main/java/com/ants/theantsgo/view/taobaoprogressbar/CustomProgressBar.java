package com.ants.theantsgo.view.taobaoprogressbar;
...
public class CustomProgressBar extends View {
     ...
     * 默认半径
     */
    private int DEFAULT_RADIUS = (int) TypedValue.applyDimension(TypedValue.COMPLEX_UNIT_DIP, 5,
            getResources().getDisplayMetrics());
    /**
     * 默认边线颜色（无用的）
     */
    private int DEFAULT_BORDER_COLOR = Color.parseColor("#DDDDDD");
    /**
     * 进度颜色
     */
     ...
     * 边线宽度
     */
    private int DEFAULT_BORDER_WIDTH = (int) TypedValue.applyDimension(TypedValue.COMPLEX_UNIT_DIP, 0,
            getResources().getDisplayMetrics());
    /**
     * 是否显示描述语言
     */
    ...
    public CustomProgressBar(Context context, AttributeSet attrs, int defStyleAttr) {
        ...
        try {
            ...
            mBorderColor = a.getColor(R.styleable.CustomProgressBar_borderColor, DEFAULT_BORDER_COLOR);
            mProgressDescColor = a.getColor(R.styleable.CustomProgressBar_progressDescColor,
    ...
    private void drawProgress(Canvas canvas) {
        ...
        if (scale <= 1) {
            ...
            path.arcTo(new RectF(left, top, left + 2 * mRadius, top + 2 * mRadius), 180,
            ...
            path.arcTo(new RectF(left, bottom - 2 * mRadius, left + 2 * mRadius, bottom),
                    180 - (float) (angle * 180 / Math.PI), (float) (angle * 180 / Math.PI));
            path.close();
            canvas.drawPath(path, mPaint);
        } else if (scale2 <= 1) {
            ...
            path.arcTo(new RectF(left, top, left + 2 * mRadius, top + 2 * mRadius), 180, 90);
            path.lineTo(right - mRadius, top);
            path.arcTo(new RectF(right - 2 * mRadius, top, right, top + 2 * mRadius), -90,
                    (float) (angle * 180 / Math.PI));
            double y = Math.pow((Math.pow(mRadius, 2) - Math.pow(a, 2)), .5) + top + mRadius;
            path.lineTo(right - mRadius + a, (float) y);
            path.arcTo(new RectF(right - 2 * mRadius, bottom - 2 * mRadius, right, bottom),
                    (float) (90 - (angle * 180 / Math.PI)), (float) (angle * 180 / Math.PI));
            path.lineTo(left + mRadius, bottom);
            path.arcTo(new RectF(left, bottom - 2 * mRadius, left + 2 * mRadius, bottom), 90, 90);
            path.close();
    ...
    private void drawProgressDesc(Canvas canvas) {
        ...
        canvas.drawText(finalProgressDesc, (int) (mWidth / 2.0 - mTextBounds.width() / 2.0),
                (int) (mHeight / 2.0 - (mTextPaint.ascent() + mTextPaint.descent()) / 2.0f), mTextPaint);
